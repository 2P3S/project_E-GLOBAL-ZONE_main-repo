  ...
  useEffect(() => {
    pending &&
      getKoreanReservation().then((res) => {
        console.log('예약 데이터 전체:', res.data);
        if (res.data && res.data.data && res.data.data.length > 0) {
          console.log('첫 번째 예약 데이터:', res.data.data[0]);
          console.log('sch_type:', res.data.data[0].sch_type);
          console.log('sch_location:', res.data.data[0].sch_location);
        }
        setData(res.data);
      });
  }, [pending]);
