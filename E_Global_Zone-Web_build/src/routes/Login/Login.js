...
const Login = () => {
      ...
      if (data) {
        if (data.data && data.data.token) {
          window.localStorage.setItem(
            "global-zone-foreigner-token",
            data.data.token
          );
          window.localStorage.setItem(
            "global-zone-loginId",
            data.data.info.std_for_id
          );
          window.localStorage.setItem(
  ...
  const handleLogin = () => {
    ...
    postForeignerLogin({ std_for_id: idValue, password: pwValue }).then(
      (res) => {
        setPending(true);
    ...
    <div className="login_content">
      <div className="head">
        <div className="head_w">
          <div className="logo">
            <img
              src="/global/img/logo_intro_globalzone.gif"
              alt="로그인 화면 로고"
            />
          </div>
          <LoginHeader />
  ...
  useEffect(() => {
    if (pending) {
      if (data) {
        if (data.data && data.data.token) {
          window.localStorage.setItem(
            "global-zone-admin-token",
            data.data.token
          );
          window.localStorage.setItem(
            "global-zone-loginId",
            data.data.info.account
          );
          window.localStorage.setItem(
            "global-zone-loginName",
            data.data.info.name
