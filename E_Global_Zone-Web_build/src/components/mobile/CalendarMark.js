  ...
  renderMonthLabel() {
    ...
    return (
      <span className="month-label">
        <span class="num">{month.format('YYYY')}</span>년 <span class="num">{month.format('M')}</span>월
      </span>
    );
  }
...
class Day extends React.Component {
  ...
  render() {
    ...
    return (
      <span
        key={date.toString()}
        className={'day' + (isCurrentMonth ? '' : ' different-month') + (date.isSame(selected) ? ' selected_' : '')}
